        ...
        <form @submit.prevent="form.post(route('car-models.update',{carModel : carModel.data.id}))" class="space-y-5">
            ...
            <div class="col-span-6 sm:col-span-3">
                ...
                <select v-model="form.brand_id" id="brand_id" name="brand_id" autocomplete="brand_id" class="mt-2 block w-full rounded-md border-0 bg-white py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                    ...
                    <option v-for="brand in brands" :value="brand.id" :selected="selected">{{ brand.name }}</option>
                </select>
                <InputError class="mt-2" :message="form.errors.brand_id"/>
            </div>

            <div v-if="props.carModel.data.image.hasOwnProperty('id')" class="grid place-content-center ">
                <InputLabel value="Old image"/>
                <img class="mt-3 object-cover border border-gray-100 shadow-sm rounded w-160 h-80"
                     :src="props.carModel.data.image.path" alt="user image"/>
            </div>
            <div>
...
const selected = (brand_id) => {
    return brand_id === props.carModel.brand;
};
