<template>
    ...
    <div class="bg-white dark:bg-gray-800">
        <div v-if="allOrders.length > 0 " class="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:pb-24">
            <div class="max-w-xl">
                <h1 class="text-2xl font-extrabold tracking-tight text-gray-900 dark:text-gray-300 sm:text-3xl">Order
                    history</h1>
            ...
            <div class="mt-16">
                ...
                <div class="space-y-20">
                    <div v-for="order in allOrders">
                        <h3 class="sr-only">Order created
                            <time>{{ order.ordered_at }}</time>
                        </h3>
                        ...
                        <div
                            ...
                            <dl class="flex-auto text-sm text-gray-600 divide-y divide-gray-200 space-y-6 dark:divide-gray-500 dark:text-gray-500 sm:divide-y-0 sm:space-y-0 sm:grid sm:grid-cols-3 sm:gap-x-6 lg:w-1/2 lg:flex-none lg:gap-x-8">
                                ...
                                <div
                                    class="flex justify-between pt-6 font-medium text-gray-900 dark:text-gray-300 sm:block sm:pt-0">
                                    <dt>Total amount</dt>
                                    <dd class="dark:text-gray-500 sm:mt-1">
                                        {{ order.subtotal }} €
                                    </dd>
                        ...
                        <table class="mt-4 w-full text-gray-500 dark:text-gray-300 sm:mt-6">
                            ...
                            <tr v-for="item in order.order_items">
                                <td class="py-6 pr-8">
                                    <div class="flex items-center">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-sm text-gray-500">
                                                {{ `${item.product.year_from} - ${item.product.year_to}` }}
                                            </div>
                                ...
                                <td class="whitespace-nowrap py-6 text-right font-medium">
                                    <Link :href="route('shop.productShow',{ product : item.product.id})"
                                          class="text-indigo-600 hover:text-indigo-700 dark:text-sky-500 dark:hover:text-sky-400">
                                        View<span class="hidden lg:inline"> Product</span></Link>
                                </td>
                            </tr>
                            </tbody>
                        </table>
        ...
        <div v-else class="w-full px-4 py-16 sm:px-6 lg:px-8 lg:pb-24">
            <div class="flex w-full justify-center">
                <h1 class="text-center text-2xl font-extrabold tracking-tight text-gray-900 dark:text-gray-300 sm:text-3xl">
                    You haven't made any orders yet</h1>
            </div>
        </div>
    </div>
...
const load = async (state) => {
    let response = await axios.get(route('orders.historyData', {page: page.value}));
